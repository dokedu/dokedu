...
func (g *Generator) LearnedCompetencesReportData(report db.Report) (*CompetencesTemplateData, error) {
	...
	userCompetences, err := g.svc.DB.UserCompetenceForCompetenceReport(ctx, db.UserCompetenceForCompetenceReportParams{
...
Outer:
	...
	competences, err := g.svc.DB.CompetencesFindByIDSortedBySortOrder(ctx, db.CompetencesFindByIDSortedBySortOrderParams{
	...
	allCompetences, err := g.svc.DB.CompetenceAll(ctx, report.OrganisationID)
	if err != nil {
		return nil, err
	}
