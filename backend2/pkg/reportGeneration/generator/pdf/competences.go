...
func (g *Generator) BaseCompetencesReportData(ctx context.Context, report db.Report) (*CompetencesTemplateData, error) {
	...
	student, err := g.svc.DB.UserFindByID(ctx, db.UserFindByIDParams{
...
func (g *Generator) CompetencesReportData(report db.Report) (*CompetencesTemplateData, error) {
	...
	competences, err := g.svc.DB.CompetenceAll(ctx, report.OrganisationID)
	if err != nil {
		return nil, err
	}

	userCompetences, err := g.svc.DB.UserCompetenceFindByUserIDForReport(ctx, db.UserCompetenceFindByUserIDForReportParams{
