...
func NewPDFReportsGenerator(svc *services.Services) *Generator {
...
func (g *Generator) pageContextData(report db.Report) (*CompetencesData, error) {
	student, err := g.svc.DB.UserFindByID(context.Background(), db.UserFindByIDParams{
	...
	teacher, err := g.svc.DB.UserFindByID(context.Background(), db.UserFindByIDParams{
		ID:             report.UserID,
		OrganisationID: report.OrganisationID,
	})
	if err != nil {
...
func (g *Generator) generateHTML(report db.Report, data any) error {
	...
	reportTemplate, err := g.svc.DB.ReportTemplateFindByName(context.Background(), db.ReportTemplateFindByNameParams{
		OrganisationID: report.OrganisationID,
