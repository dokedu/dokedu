...
func (g *Generator) preloadAllEntriesReportData(ctx context.Context, o db.Organisation) (*internData, error) {
	...
	data.Entries, err = g.svc.DB.EntriesAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}
	...
	data.EntryEvents, err = g.svc.DB.EntryEventsAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}
	...
	data.Events, err = g.svc.DB.EventsAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}

	data.EventCompetences, err = g.svc.DB.EventCompetencesAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}

	data.Competences, err = g.svc.DB.CompetencesAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}
	...
	data.UserCompetences, err = g.svc.DB.UserCompetencesAll(ctx, o.ID)
	if err != nil {
		return nil, err
	}
...
func (g *Generator) AllEntriesReportData(report db.Report) (*AllReportData, error) {
	...
	organisation, err := g.svc.DB.GLOBAL_OrganisationFindByID(ctx, report.OrganisationID)
	if err != nil {
		return nil, err
	}
	...
	for i := range data.Entries {
		...
		dateFormatted := data.Entries[i].Date.Time.Format("02.01.2006")
