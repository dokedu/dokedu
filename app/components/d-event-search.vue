...
import type { DEvent } from "~/types/models"
...
const filtered = computed(() => {
  let items: DEvent[] = events.value

  // filter by search
  if (search.value) {
    items = items.filter((c) => c.title.toLowerCase().includes(search.value.toLowerCase()))
  }

  return items
})

function onClick(event: DEvent) {
  return emit("toggle", event)
}
...
<template>
  <div class="flex h-[500px] w-full flex-col">
    ...
    <div v-if="events" class="flex-1 divide-y divide-neutral-200 overflow-auto">
      <div
        ...
        @click="onClick(event)"
      >
        <div class="grid grid-cols-2 items-center gap-2.5" style="grid-template-columns: auto 1fr">
          <CircleCheckIcon v-if="selected.includes(event.id)" class="size-4 text-blue-700" />
          <CircleIcon v-else class="size-4 text-neutral-400" />
          <div class="text-sm text-neutral-700">{{ event.title }}</div>
        </div>
      </div>
